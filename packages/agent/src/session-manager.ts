...
export class SessionManager {
   ...
   */
  async checkConsistency(session: AgentSession, projectRoot: string): Promise<ConsistencyCheckResult> {
    ...
    if (externallyChanged.length > 0) {
      return {
        ...
        warning: `The project changed externally since the last checkpoint in ${externallyChanged.length} file(s).`,
      };
    }
